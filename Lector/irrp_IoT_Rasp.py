...
EstadoControl={
         "Estado":"Inactivo","Mensaje":"","Confirmado":False,"Guardado":False
         }
RutaDeJson="Lector/EstadoControl.json"
last_tick = 0
in_code = False
code = []
fetching_code = False
...
if args.record: # Record.
   ...
   print("Recording")
   EstadoControl["Estado"]="Activo"
   for arg in args.id:
      
      
      EstadoControl["Mensaje"]="Press key"
      ...
      if CONFIRM:
         ...
         while not done:
            ...
            if the_same:
               ...
               records[arg] = press_1[:]
               print("Okay")
               EstadoControl["Mensaje"]="Okay"
               EstadoControl["Confirm"]=True
               with open(RutaDeJson, 'w') as file:
                  json.dump(EstadoControl, file, indent=4)
               time.sleep(0.5)
            else:
               ...
               if tries <= 3:
                  print("No match")
                  EstadoControl["Mensaje"]="No Match"
                  
               else:
                  print("Giving up on key '{}'".format(arg))
                  EstadoControl["Mensaje"]="Giving up on key"
                  done = True
               with open(RutaDeJson, 'w') as file:
                        json.dump(EstadoControl, file, indent=4)
               time.sleep(0.5)
